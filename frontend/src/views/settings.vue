    ...
    <div class="card">
      <div class="card-header">
        <div class="card-header-title">
          <span v-if="user.id != null">Settings for {{ user.username }}</span>
          <span v-else>Settings for Guest User</span>
        </div>
      </div>
      <div class="card-content">
        <div class="field">
          <div class="control">
            <label for="bisTheme">Colour Scheme</label>
        ...
        <div class="field">
          <button class="button is-success" @click="save" v-if="user.id !== null">Save</button>
          <button class="button is-success" v-else disabled>Save</button>
        </div>
...
export default class Settings extends SavageAimMixin {
  ...
  async save(): Promise<void> {
    const body = JSON.stringify({ theme: this.user.theme })
    try {
