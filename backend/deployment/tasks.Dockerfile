...
EXPOSE 443
ENTRYPOINT gunicorn --bind=0.0.0.0:443 -k gevent --timeout 600 --access-logfile - --log-file - --log-level info --capture-output --enable-stdio-inheritance backend.wsgi

# Use this one if we need to update our tasks
